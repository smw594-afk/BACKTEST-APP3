  ...
  if (cachedData) {
    ...
    try {
      ...
      const dbResult = await env.DB.prepare(
        "SELECT date, open, close FROM stock_prices WHERE ticker = ? AND date >= ? AND date <= ? ORDER BY date ASC"
      ).bind(ticker, normalizeStockDate(sDateStr), normalizeStockDate(eDateStr)).all();
...
// 📈 1. 야후 파이낸스 주가 수집 프록시 API (기존 Vercel 기능 고속 흡수)
  ...
  async scheduled(controller, env, ctx) {
    // ⏰ 백그라운드 스케줄러: 앱을 켜지 않아도 지정된 시간에 주가를 자동 저장
    const tickers = ["SOXL", "QQQ"];
    const pad = (n) => String(n).padStart(2, '0');
    const formatDateNY = (d) => {
    ...
    console.log("[스케줄러] 주가 자동 수집 크론 트리거 가동 시작");

    const fetchAndCache = async (ticker) => {
      try {
        const nowSec = Math.floor(Date.now() / 1000);
        const sevenDaysAgoSec = nowSec - (86400 * 7); // 최근 7일치 가져와 증분 업데이트

        const yUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${ticker}?period1=${sevenDaysAgoSec}&period2=${nowSec}&interval=1d`;
        const yResponse = await fetch(yUrl, {
          headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" }
        });

        if (yResponse.ok) {
          const rawData = await yResponse.text();
          ...
          if (res.chart && res.chart.result && res.chart.result[0]) {
            ...
            const statements = [];
            for (let i = 0; i < ts.length; i++) {
              if (cls[i] !== null && ops[i] !== null && ts[i]) {
                const dObj = new Date(ts[i] * 1000);
                const dateStr = normalizeStockDate(formatDateNY(dObj));
                statements.push(
                  env.DB.prepare("INSERT OR REPLACE INTO stock_prices (ticker, date, open, close) VALUES (?, ?, ?, ?)")
                    .bind(ticker, dateStr, Math.round(Number(ops[i]) * 100) / 100, Math.round(Number(cls[i]) * 100) / 100)
                );
              }
            }
            ...
            if (statements.length > 0) {
              await env.DB.batch(statements);
              console.log(`[스케줄러] D1 DB에 ${ticker} 주가 ${statements.length}건 자동 업데이트 완료`);
            }
          }
        }
      } catch (err) {
        console.error(`[스케줄러] ${ticker} 주가 자동 수집 실패:`, err);
      }
    };

    const runAll = async () => {
      await Promise.all(tickers.map(ticker => fetchAndCache(ticker)));
